// mobile/app/(admin)/settings.jsx
import { useState, useEffect } from 'react';
import {
  View, Text, ScrollView, TouchableOpacity,
  StyleSheet, Alert, TextInput, ActivityIndicator, Platform, StatusBar,
  Dimensions, Image
} from 'react-native';
...
export default function AdminSettingsScreen() {
  ...
  // Custom workable preference settings
  ...
  const handleUpdateSettings = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        Alert.alert('Success', 'Admin preferences updated successfully!');
      }
    } catch (error) {
  ...
  const pickProfilePicture = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        await uploadProfilePictureToServer(result.assets[0]);
      }
    } catch (error) {
  ...
  const uploadProfilePictureToServer = async (asset) => {
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* Preferences Section */}
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: theme.textSub }]}>Canteen & Management Preferences</Text>
          <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            ...
            <View style={styles.prefRow}>
              <View style={styles.prefLeft}>
                ...
                <View>
                  <Text style={[styles.prefLabel, { color: theme.text }]}>Order Alert Notifications</Text>
                  <Text style={[styles.prefDesc, { color: theme.textSub }]}>Receive real-time alerts when new orders are placed</Text>
                </View>
              </View>
              <TouchableOpacity 
                activeOpacity={0.8}
            ...
            {/* Input: Phone */}
            <Text style={[styles.label, { color: theme.textSub, marginTop: 12 }]}>Admin Support Phone Number</Text>
            ...
            {/* Input: Address */}
            <Text style={[styles.label, { color: theme.textSub, marginTop: 16 }]}>Admin Office / Pickup Desk Address</Text>
            <View style={[styles.inputRow, { backgroundColor: theme.inputBg, borderColor: theme.border, alignItems: 'flex-start', paddingVertical: 10 }]}>
              ...
              <TextInput
                ...
                placeholder="Canteen Counter, Floor, Building Info..."
                placeholderTextColor={theme.textMuted}
                value={address}
        ...
        {/* Security Section */}
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: theme.textSub }]}>Security & Password</Text>
        ...
        {/* Account Actions */}
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: theme.textSub }]}>Account Management</Text>
          <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <TouchableOpacity
      ...
      {isPhone && (
        <View style={styles.bottomNav}>
          {[
            ...
            { icon: 'settings',   iconO: 'settings-outline',   label: 'Settings',  active: true,  route: '/(admin)/settings' },
          ].map(n => (
            <TouchableOpacity
              key={n.label}
            ...
            >
              <Ionicons
                ...
                size={22}
                color={n.active ? '#E8820C' : '#666'}
              />
              <Text style={[styles.bottomNavLabel, n.active && styles.bottomNavLabelActive]}>
                {n.label}
              </Text>
            </TouchableOpacity>
          ))}
